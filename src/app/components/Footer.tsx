      ...
      <div className="container-default">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="flex gap-8">
                ...
                <a key={i} href={linkTargets[i] || "#"} className="hover:text-brand-accent transition-colors font-medium">{link}</a>
                ))}
            </div>
            ...
            <div className="text-right flex flex-col items-end gap-2">
                ...
                <div className="flex gap-6 justify-end items-center">
                    {t.footer.legal.map((item, i) => {
                        ...
                        if (isTerms) {
                          return (
                            <Link key={i} to="/terms" className="hover:text-brand-accent transition-colors">
                              {item}
                            </Link>
                          );
                        }
                        return (
                          <a key={i} href={legalTargets[i] || "#legal"} className="hover:text-brand-accent transition-colors">
                            {item}
                          </a>
                        );
                    })}
