...
export function TrustCenter() {
  return (
    <section className="section-spacing bg-brand-background-primary border-t border-brand-border" id="legal">
            ...
            <ul className="space-y-3 text-sm text-brand-text-secondary">
              {securityHighlights.map((item) => (
                <li key={item} className="flex items-start gap-3">
                  <Lock className="w-4 h-4 text-brand-accent mt-1" />
                  <span>{item}</span>
                </li>
              ))}
            </ul>
          ...
          <div className="lg:w-2/3">
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="privacy" className="border-b border-brand-border">
                <AccordionTrigger className="text-left text-lg font-semibold text-brand-text-primary hover:text-brand-accent hover:no-underline py-6 transition-colors">
